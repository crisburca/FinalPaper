...
The data is sourced from tr56 (cite), containing information on players and tournaments played in the World Series from years 2108 to 2023 with gaming statistics for each match.

The analysis of the data is done using R and packages @citeR, cleaned using @citeTidyverse and plots, graphs and tables were created using ...
...
sds <- sapply(model_data[, c("golddiffat10", "xpdiffat10"
...
coefficients_scaled <- coef(blue_results)

# Scale back the coefficients using the stored standard deviations
coefficients_rescaled <- coefficients_scaled / sds

# Display the rescaled coefficients
coefficients_rescaled <- exp(coefficients_rescaled)
```
